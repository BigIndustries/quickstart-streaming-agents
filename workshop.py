...
Run by each workshop PARTICIPANT after the organizer has run `uv run deploy`.
...
def _load_core_outputs(root: Path) -> dict:
    ...
    if not state_path.exists():
        ...
        print("The workshop organizer must run `uv run deploy` first.")
        sys.exit(1)
...
def _save_user_credentials(root: Path, username: str, email: str, kafka_key: str, kafka_secret: str, sr_key: str, sr_secret: str, core: dict) -> None:
    path = root / f"credentials-{username}.env"
    lines = [
        f"# Workshop credentials for {email} — generated by uv run workshop",
...
def main():
    ...
    kafka_key, kafka_secret = create_api_key(
        display_name=f"workshop-{username}-kafka-key",
    ...
    sr_key, sr_secret = create_api_key(
        display_name=f"workshop-{username}-sr-key",
    ...
    print(f"  Topic / table prefix: {username}_")
    print(f"  Kafka API key       : {kafka_key}")
    print(f"  Credentials file    : credentials-{username}.env")
